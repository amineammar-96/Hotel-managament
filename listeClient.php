...
<body class="bg-light">
    ...
    <div class="row py-6">
        ...
        $nom = $_POST["nomClient"];
        $prenom = $_POST["pnomClient"];
        $refClient = $_POST["refClient"];
        $email = $_POST["addEmailClient"];
        $numTel = $_POST["numTelClient"];
...
$new_date1 = date('Y-m-d', strtotime($date11));
        ...
        if($new_date1 == null){$new_date1="";}

        


        if (($nom !="")||($prenom !="")||($new_date1 !="")
        ||($refClient !="")||($email !="")||($numTel !=""))
         {
        ...
        if($refClient !=""){echo " Reférence reservation :   " .$refClient;}
        if($new_date1 !=""){echo "<br> org :   " .$new_date1;}

        echo "</ul>";

        
        $sql = "SELECT  * FROM client
        WHERE nameClient LIKE '%$nom%' AND pnameClient LIKE '%$prenom%' AND emailClient LIKE '%$dateCheckIn%' 
        AND telClient LIKE '%$dateCheckOut%'  
       AND dateCreation LIKE '%$numTel%'"; 
